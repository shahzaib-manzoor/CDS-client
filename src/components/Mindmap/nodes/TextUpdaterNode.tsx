...
function TextUpdaterNode({ data }: TextUpdaterNodeProps) {
  ...
  const getOptionsByType = (type: string | undefined) => {
    switch (type) {
      case "number":
        return (
          <>
  ...
  const positionHandle = (index: number) => {
    const calculated =
      data.conditions?.length * 100 + 210 + index * 80;
    return calculated;
  };
  ...
  return (
    ...
    >
      ...
      {conditions?.map((condition: any, index: number) => (
        <div
          key={index}
          className="p-4 border-b border-gray-200 bg-[#f0f0f0] rounded-lg flex items-center mb-3"
        >
          <div className="flex-1 p-2 border rounded-md mr-2.5">
            <ReactSelect
              options={configOptions}
              placeholder="Search configurations..."
              className="nodrag nopan flex-1"
              value={configOptions.find(
                (option) => option.value === condition.key
              )}
              isSearchable
              onChange={(option: any) =>
                updateConditionRow(index, "key", option.value)
              }
            />
          </div>
          <select
          ...
          >
            {getOptionsByType(
              configOptions.find((option) => option.value === condition.key)
                ?.type
            )}
          </select>
          <input
